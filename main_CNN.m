...
for i = 1: length(train_image_path_vector)
    ...
    s = sprintf(' %d / %d \n time used: %.2f \n time remains %.2f \n', i,length(train_image_path_vector), toc, remain_time);    fprintf(s);   
    fprintf(s);  
...
% time tracking
  ...
  % XVal: testing data
    XVal = zeros(41,41,1,length(valData.BW));
    
    for j = 1:length(valData.BW)
        XVal(:,:,1, j) = valData.BW(j).image;
    end
    ...
    % TVal is true label
    [YVal,score] = classify(trainedNet,XVal);
...
save('/Users/qingdai/Desktop/fos_detection/model/model_CNN(Gray, I_equalized).mat' , ...
    'trainedNet');
...
for i = 1:length(test_image_path_vector)
   ...
   % section
    [ Labels, BW_patch, ~, Gray_patch]...                       
        = create_pixel_features(test_image_path_vector{i}, test_tag_path_vector{i}, 'cfos');
         
    % concatenate true labels among iterations
    test_Labels = [test_Labels; Labels];
    
    
    XTest = zeros(41,41,1,length(BW_patch));
    
    for j = 1:length(BW_patch)
        XTest(:,:,1, j) = BW_patch(j).image;
    end
    ...
    % YVal is predict label
    % TVal is true label
    [YTest,s] = classify(trainedNet,XTest);
    YTest = double(YTest);
    YTest(YTest == 1) = 0;
    YTest(YTest == 2) = 1;
