function [Feature_vector, Label_vector, num_of_candidates, L2] = extract_feature_and_load_tags(image_path, tag_path, target)
...
for i = 1:size(raw,1)
    if (contains(raw(i,2), target) || contains(raw(i,2), 'colabel'))
        load_tags = [load_tags;horzcat(num(i-1,1), num(i-1,3:4))];
    end
end

% load tags, pair tags with candidates
[num_of_positive_signals,positive_signals] = match_tags(load_tags, Candidate_properties);

%%tags-read, centroid-green
figure;
imshow(I_bw);
hold on;
plot(load_tags(:,2), load_tags(:,3),'r*');  % manually tag red
